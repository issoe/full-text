...
function BaseNav({ active }) {
  ...
  return (
    <div className="w-[80%] h-auto mx-auto my-4">
      ...
      <div className='font-bold text-lg mt-2'>Top hits</div>
      <div className="w-full h-[1px] bg-[#FFF]"></div>
            {topHits?.map((item, index) => {
              return (
                <Paper key={index} keyword={item.keyword}>
                  <div className="relative">
                    <div className='inline-block bg-[#6465F1] px-2 m-1 rounded-full'>{index + 1}</div>
                    {item.keyword}
                    <strong className="absolute right-[0px]">{item.count}</strong>
                  </div>
                </Paper>
              );
            })}
